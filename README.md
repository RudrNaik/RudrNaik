# 👋 Heyo! I'm Rudra Naik
- BS in Computer Science @ Iowa State
- Nuclear Option Modder
- Tabletop/Unity Game Developer
- Concept/Character artist
# Connect with me!
- 📧 rudramnaik@gmail.com
- 🐦 https://x.com/Spinypine2
- 💼 https://www.linkedin.com/in/rudra-naik-8b1878279/
# Current Projects
- CALLICOM
  - Website developed as a GM and Player tool for CALAMARI, my tactical (digital) tabletop roleplaying game, built for discord and other online platforms.
  - MERN Stack
- PRISUM Software 
  - Developing Strategy and Embedded Systems software for PRISUM solar car team @ Iowa State.
  - C/C++/Python
- ACES HIGH
  - Website developed for the Aces High Team and their TTRPG under the same name. Developed as a player compendium and character manager. 
- SEAD 
  - Unity based, side scrolling, 2d flight action game. In development.
  - C#/Unity Engine
- Nuclear Option Modding
  - Bepinex 5 Modding for Shockfront's Flight Action Simcade game, Nuclear Option. All mods are hosted on NOMNOM and accessible via NOMM directly. or download the .dll files here via the latest releases!
  - C#/Bepinex 5
# ✅ Completed Projects
- NerdMarket 
  - Gradle/Maven/Springboot Android App to allow users to store and track their trading card portfolio over Pokemon, Magic The Gathering, and Yu-Ghi-Oh
- Chargeboard (P15 Eliana)
  - C++ Embedded Software to Enable PRISUM's P15 Eliana to utilize SAE J1772 EVSE systems to charge the onboard battery for the 2024 American Solar Challenge and Formula Sun Grand Prix. 
  -  Placed 3rd in MOV class
# Current Relevant Coursework
Currently Learning:
- Summer :)
# ✅ Completed Coursework
@ Iowa State University:
- COMS 363 | Introduction to Database Management Systems
- COMS 331 | Theory of Computing
- COMS 327 | Advanced Programming Techniques
- COMS 321 | Introduction to Computer Architecture and Machine Level Programming
- COMS 319 | Construction of User Interfaces
- COMS 311 | Intro to the Design and Analysis of Algorithms
- COMS 309 | Software Development Practices
- COMS 230 | Discrete Computational Structures
- COMS 228 | Data Structures & Algorithms
- COMS 227 | Object Oriented Programming
- COMS 127 | Introduction to Computer Programming
